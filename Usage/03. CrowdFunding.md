...
4. 배포된 계약을 통해 다음과 같은 기능들을 테스트한다:
    - 새로운 캠페인 생성 (`launch`)
    ![crowdfunding](../image/cflaunch.png)
    - ERC20 Token Smart Contract에서 `approve` function을 호출하여 `CrowdFund` Smart Contract가     Token을 전송할 수 있도록 승인
    ![crowdfunding](../image/cfapprove.png)
    - 캠페인에 기부 (`pledge`)
    ![crowdfunding](../image/cfpledge.png)
    - 기부 취소 (`unpledge`)
    ![crowdfunding](../image/cfunpledge.png)
    - 목표 금액 달성 시 자금 인출 (`claim`)
    ![crowdfunding](../image/cfclaim.png)
    - 목표 금액 미달성 시 기부금 환불 (`refund`)
    ![crowdfunding](../image/cfrefund.png)
